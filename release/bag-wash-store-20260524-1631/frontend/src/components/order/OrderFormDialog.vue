...
watch(
  ...
  async (open) => {
    if (!open) return
    washServiceOptions.value = await serviceStore.fetchEnabledForSelect()
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px" class="order-form">
        ...
        <el-form-item label="洗护项目" prop="washServices">
          <el-checkbox-group v-model="form.washServices">
            <el-checkbox v-for="opt in washServiceOptions" :key="opt.value" :label="opt.value" border>
              {{ opt.label }}
            </el-checkbox>
          </el-checkbox-group>
          <p v-if="!washServiceOptions.length" class="upload-tip">请先在「洗护服务配置」中新增并启用服务项目</p>
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
